 ...
 */
export async function crearVariante(principalCodigo: string, formData: FormData) {
  const s = await exigirSesionActiva()
  if (!puedeEditarProduccion(s)) {
 ...
 */
export async function definirVariantesRequeridas(codigo: string, formData: FormData) {
  const s = await exigirSesionActiva()
  if (!puedeEditarProduccion(s)) {
    throw new Error('Sólo el equipo SIED arma los seminarios optativos')
  }
...
export async function desvincularVariante(codigo: string) {
  const s = await exigirSesionActiva()
  if (!puedeEditarProduccion(s)) {
    throw new Error('Sólo el equipo SIED arma los seminarios optativos')
  }
